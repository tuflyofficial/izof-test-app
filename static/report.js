document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!reportDataString) {
        reportContent.innerHTML = '<p style="color: red;">리포트 생성 데이터가 없습니다. 이전 페이지로 돌아가세요.</p>';
        return;
    }
    sessionStorage.removeItem('reportData'); // 사용 후 즉시 데이터 삭제
    ...
    lines.forEach(line => {
        const parts = line.trim().split(/\s+/); // 탭이나 여러 공백으로 분리
        if (parts.length >= 3) {
    ...
    let aiInsightHtml = '';
    const maxScore = Math.max(...trainingScores);
