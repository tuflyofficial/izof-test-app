...
def summarize_all():
    if not model: return jsonify({"error": "API 키 미설정"}), 500
    data = request.json
    if not all(k in data for k in ['raw_data', 'schedule_text', 'ai_insight_html']):
        return jsonify({"error": "리포트 데이터 부족"}), 400

    raw_data = data['raw_data']
    schedule_text = data['schedule_text']
    ai_insight_html = data['ai_insight_html'] # JS로부터 미리 만들어진 Insight HTML

    # ▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼ 계산/로직 부분이 제거된, 훨씬 단순하고 안정적인 프롬프트 ▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼
    prompt = f"""
    당신은 주어진 정보들을 사용하여 깔끔한 HTML 보고서를 만드는 서식 전문가입니다.
    아래 주어진 데이터들을 사용하여 하나의 완성된 HTML 리포트를 생성해주세요.
